...
class Solution {
  struct Graph {
    ...
    Graph(const size_t& num_of_edges) : edge_list() {
      edge_list.reserve(num_of_edges * 2);
    }
 ...
 private:
  ...
  void BFS(bool& norm, const size_t& start, const size_t& finish) {
    ...
    size_t idx = 1;
    size_t id1 = 0;
    for (; id1 < idx && arr1[finish] == INT32_MAX; ++id1) {
      for (const unsigned long& j : arr4[arr3[id1 - 1]]) {
        if (arr1[graph_body_.get_finish(j)] == INT32_MAX &&
            graph_body_.get_thd(j) < graph_body_.get_cap(j)) {
          arr3[idx] = graph_body_.get_finish(j);
          ++idx;
