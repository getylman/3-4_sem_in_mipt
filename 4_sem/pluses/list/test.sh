...
python3 banned_words_checker.py --solution=list.hpp --banned-words=banned_words.json
if [[ ! $? -eq 0 ]]
then
...
clang-tidy -extra-arg=-std=c++20 -quiet list.hpp
if [[ ! $? -eq 0 ]]
then
...
clang-format -i -style='{BasedOnStyle: Google, DerivePointerAlignment: false, PointerAlignment: Left, AlignOperands: true}' list.hpp &&
git diff --ignore-submodules --color list.hpp > diff &&
cat diff
if [[ -s diff ]]
then
